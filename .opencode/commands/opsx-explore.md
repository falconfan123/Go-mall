...
## Explore 落盘（Reflection Harness 必做）

**探索结束若产生了 change 需求，必须把讨论结论落盘为 `openspec/changes/<change-name>/explore-brief.md`**，作为提案审核的持续性 checklist baseline。简单需求未经过 explore 讨论时可不生成。

原因：explore 讨论不落盘，新开会话或上下文超限后细节全丢；且反思智能体在子会话中调起时拿不到讨论背景。

落盘时机与方式：
1. 探索结束、用户决定进入 propose 时：先 `openspec new change "<name>"` 正规创建骨架（不要手建目录），再把 explore-brief.md 写入 change 目录。
2. explore-brief.md 结构建议：
   - **背景** — 现状调查结论（有实证就写实证：端口、配置、实际运行状态）
   - **目标 / 非目标** — 用户视角的收益与明确排除项
   - **待决策点** — 探索中浮现的 D1/D2... 选项与倾向
   - **Checklist** — 后续提案审核时逐项核对的基线清单
3. 后续 propose 的每个制品（proposal/design/specs/tasks）过反思审核时，审核方都要以 explore-brief.md 为需求基线核对。
