...
func (l *CreateOrderLogic) CreateOrder(in *order.CreateOrderRequest) (*order.OrderDetailResponse, error) {
	...
	// 新增：通知 checkout 服务订单已创建，清除预订单锁
	_, err = l.svcCtx.CheckoutRpc.UpdateStatus2Order(l.ctx, &checkout.UpdateStatusReq{
		PreOrderId: dto.PreOrderID,
		UserId:     int32(dto.UserID),
	})
	if err != nil {
		// 仅记录错误，不影响主流程
		l.Logger.Errorw("call checkout rpc UpdateStatus2Order failed", append(l.logContext(dto), logx.Field("err", err))...)
	}
	...
	res.Order = &order.Order{
		...
		OrderStatus:    order.OrderStatus_ORDER_STATUS_CREATED,
		PaymentStatus:  order.PaymentStatus_PAYMENT_STATUS_NOT_PAID,
	}
	return res, nil
}
