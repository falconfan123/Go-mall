...
const (
	...
	Coupons_UseCouponRollback_FullMethodName = "/coupons.Coupons/UseCouponRollback"
)
...
type CouponsClient interface {
	...
	// 使用优惠券回滚（Saga 补偿分支）：Saga 补偿回调与 action 共用同类型 payload
	// （UseCouponReq），故补偿方法必须接受 UseCouponReq（实施期发现，design 决策 2）。
	UseCouponRollback(ctx context.Context, in *UseCouponReq, opts ...grpc.CallOption) (*EmptyResp, error)
}
...
func (c *couponsClient) UseCouponRollback(ctx context.Context, in *UseCouponReq, opts ...grpc.CallOption) (*EmptyResp, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(EmptyResp)
	err := c.cc.Invoke(ctx, Coupons_UseCouponRollback_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
type CouponsServer interface {
	...
	// 使用优惠券回滚（Saga 补偿分支）：Saga 补偿回调与 action 共用同类型 payload
	// （UseCouponReq），故补偿方法必须接受 UseCouponReq（实施期发现，design 决策 2）。
	UseCouponRollback(context.Context, *UseCouponReq) (*EmptyResp, error)
...
func (UnimplementedCouponsServer) CalculateCoupon(context.Context, *CalculateCouponReq) (*CalculateCouponResp, error) {
	return nil, status.Error(codes.Unimplemented, "method CalculateCoupon not implemented")
}
...
func (UnimplementedCouponsServer) UseCouponRollback(context.Context, *UseCouponReq) (*EmptyResp, error) {
	return nil, status.Error(codes.Unimplemented, "method UseCouponRollback not implemented")
}
...
func RegisterCouponsServer(s grpc.ServiceRegistrar, srv CouponsServer) {
	// If the following call panics, it indicates UnimplementedCouponsServer was
...
func _Coupons_UseCouponRollback_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(UseCouponReq)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(CouponsServer).UseCouponRollback(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: Coupons_UseCouponRollback_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(CouponsServer).UseCouponRollback(ctx, req.(*UseCouponReq))
	}
	return interceptor(ctx, in, info, handler)
}
...
var Coupons_ServiceDesc = grpc.ServiceDesc{
	...
	Methods: []grpc.MethodDesc{
		...
		{
			MethodName: "UseCouponRollback",
			Handler:    _Coupons_UseCouponRollback_Handler,
		},
	},
