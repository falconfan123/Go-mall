...
func (l *ListActivitiesLogic) ListActivities(in *adminpb.ListActivitiesRequest) (*adminpb.ListActivitiesResponse, error) {
	activities, total, err := db.ListActivities(l.svcCtx.DB, in.Page, in.PageSize, in.Status)
	if err != nil {
		return &adminpb.ListActivitiesResponse{
			StatusCode: 500,
			StatusMsg:  "failed to list activities: " + err.Error(),
		}, nil
	}

	pbActivities := make([]*adminpb.Activity, len(activities))
