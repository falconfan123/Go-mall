...
- **明确排除**：Mock Consistency 不纳入（A1 已绿）；**不改业务代码、不改测试断言**。**唯一例外（经用户拍板，2026-09-14）**：① **D6** 删除 `dal/model/inventory/inventorymodel.go` 中 1 行不可达死代码（语义等价，`go vet` unreachable 报错源，达成 C2 必要；来源 Initial commit 254330c 存量缺陷）；② **D7** 修正 `test/rpc/internal/testenv/testenv.go` 本地模式 gRPC target scheme（裸 `ip:port` → `passthrough:///127.0.0.1:port`，grpc-go 默认 resolver dns 化导致 zero addresses，语义不变、不放宽断言、不 skip；起栈修复后才暴露的存量测试基座问题）；③ **D8 测试基座**（起栈修复后首次执行的存量测试基座问题，不改断言、不 skip）：order 测试包 3 文件 pb import 补 `order` 别名（对齐 create_test.go 既有写法）；`user_coupons` 测试种子缺失（coupons Lock/Unlock/Use 需 user 1 持有种子券行）→ 补种子；inventory 高并发测试 flaky 判定（重跑 3 次 1 败 2 过）→ 归 flaky 记录，不阻塞。