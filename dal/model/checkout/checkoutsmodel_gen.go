...
type (
	...
	Checkouts struct {
		...
		AddressId      uint64         `db:"address_id"`      // 收货地址ID
	}
)
...
func (m *defaultCheckoutsModel) Insert(ctx context.Context, data *Checkouts) (sql.Result, error) {
	...
	ret, err := m.conn.ExecCtx(ctx, query, data.PreOrderId, data.UserId, data.CouponId, data.OriginalAmount, data.FinalAmount, data.Status, data.ExpireTime, data.AddressId)
	return ret, err
}
...
func (m *defaultCheckoutsModel) Update(ctx context.Context, data *Checkouts) error {
	...
	_, err := m.conn.ExecCtx(ctx, query, data.UserId, data.CouponId, data.OriginalAmount, data.FinalAmount, data.Status, data.ExpireTime, data.AddressId, data.PreOrderId)
	return err
}
